...
{
	name = "[TF2] SandBox - Search System", 
...
{
	RegAdminCmd("sm_sbs", Command_Search, 0, "sm_");
	RegAdminCmd("sm_sbsearch", Command_Search, 0, "Open SearchSystem menu");
}
...
public Action Command_Search(int client, int args)
{
	if(IsValidClient(client) && IsPlayerAlive(client))
	{
		if(args == 1)
		{
			...
			SetMenuTitle(g_hPropMenu[client], "TF2Sandbox - Search System v%s\n \nSearch Result:", PLUGIN_VERSION);
